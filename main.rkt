...
(define fileName (string-append 
                    (last 
                        (string-split argFolder "/") ) 
                    ".asm"))
...
(define (analyze fileLine)
    ...
    (let* (
        [splitted (string-split fileLine " ")]
        [command (car splitted)]
        )
    (cond 
...
(define outputfile (open-output-file (string-append argFolder "/" fileName) 
        #:exists 'replace
         #:replace-permissions? #t
        ))
(for  ([ i (map path->string (directory-list argFolder))]     
    #:when (string-suffix?  i ".vm") )
